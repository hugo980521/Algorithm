...
public class Test43 {
    ...
    private static void probability(int number, int[] probabilities, int max) {
        for (int i = 1; i <= max; i++) {
            probabilityDetail(number, number, i, probabilities, max);
        }
    }
     ...
     * @param max           色子的最大值
     */
    private static void probabilityDetail(int original, int current, int sum, int[] probabilities, int max) {
        if (current == 1) {
            probabilities[sum - original]++;
        } else {
            for (int i = 1; i <= max; i++) {
                probabilityDetail(original, current - 1, i + sum, probabilities, max);
            }
        }
    }

...
import algrightm.sort.heap_sort.java.HeapSort;

public class TwoNProblem {
    // 题目：任意2n个整数，从其中选出n个整数，使得选出的n个整数和同剩下的n个整数之和的差最小。
    public static void main(String[] args) {
//        int A[] = { 1, 2, 3, 5, 7, 8, 9 };
//        int A[] = { 1, 2, 3, 5, 7, 8, 1000 };
        int A[] = { 1, 2, 3, 5, 7, 8, 50 };
...
//        getIndex(B);
    ...
    public  static  int getIndex(int[] numArr){
        if(null==numArr || numArr.length<2){
            return -1;
        }
        int len=numArr.length;
        HeapSort.heapSortAsc(numArr,len);
        ...
        int threadHold=sum/2;
        int index=half;
        while(sumN<threadHold){
            sumN=sumN-numArr[index-half]+numArr[index];
            index++;
        }
        System.out.println("index="+index);
        if(len%2==1){
            for(int i=index-half;i<index;i++){
                System.out.println(numArr[i]+"  ");
            }
        }else{
            for(int i=index-half-1;i<index-1;i++){
                System.out.println(numArr[i]+"  ");
            }
        }
